...
const services = credentials => APIS.reduce((acc, { service, type, baseUrl }) => ({
  ...acc,
  [service]: new StoryblocksApi(baseUrl, credentials, [
    ...
    ...(type === 'audio' ? [{ name: 'subcategories', endpoint: e`/api/v1/stock-items/categories/${'category'}/subcategories` }] : []),
